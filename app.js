...
function parseReadings(text){
  ...
  for(const part of parts){
    ...
    if(found){
      current=found;
      (rest.match(/\d+/g)||[]).forEach(n=>out.push({book:current,chapter:n}));
    }else if(current){
      (part.match(/\d+/g)||[]).forEach(n=>out.push({book:current,chapter:n}));
    }
  }
...
function renderReading(){
  ...
  reading.innerHTML=parseReadings(text).map(r=>{
