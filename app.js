...
function renderReadings(data, date) {
  const official = data?.officialUrl || officialUrl(date);
  const readings = Array.isArray(data?.readings) ? data.readings : [];
  const unmapped = Array.isArray(data?.unmapped) ? data.unmapped : [];

  if (!readings.length) {
    el("readings").innerHTML = `
      <a class="chapter" href="${official}" target="_self">
  ...
  el("readings").innerHTML =
    readings.map(item => `
      <a class="chapter" href="${item.url}" target="_self">
    ...
    (unmapped.length ? `
      <a class="chapter" href="${official}" target="_self">
        <span>⚠️ Открыть оставшиеся чтения на официальной странице</span>
        <span>›</span>
      </a>
    ` : "");
}
